...
# Part 1B: Base BEKK-GARCH Estimation (Without Structural Breaks)

# Base BEKK function with QMLE estimation
base_bekk <- function(returns, vix_lag) {
  T <- nrow(returns)
  K <- ncol(returns)
  n_params <- 11  # Reduced from 15 as we removed structural break parameters
  ...
  # Modified initial parameters (removed structural break parameters)
  init_params <- c(
    init_C[1,1], init_C[2,1], init_C[2,2],  # C matrix
    0.15, 0.05, 0.05, 0.15,   # A matrix
    0.85, 0.05, 0.05, 0.85    # G matrix
  )
  
  # Modified parameter bounds (removed bounds for structural break parameters)
  lower_bounds <- c(
  ...
  # Modified QMLE likelihood function for base model
  negloglik <- function(params) {
    tryCatch({
      storage$last_params <- params
      
      # Reconstruct matrices from parameters (without structural break matrices)
      C <- matrix(0, 2, 2)
      ...
      for(t in 2:T) {
        ...
        # Basic BEKK equation without structural break components
        H_next <- crossprod(C) + 
          A %*% (eps_t_1 %*% t(eps_t_1)) %*% t(A) + 
          G %*% storage$H[,,t-1] %*% t(G)
  ...
  cat("\nStarting Base BEKK-GARCH optimization...\n")
  ...
  # Calculate robust standard errors
  scores <- calc_base_scores(final_params, scaled_returns)
...
# Modified helper function to calculate scores for base model
calc_base_scores <- function(params, returns) {
  T <- nrow(returns)
  ...
  # Reconstruct matrices (without structural break matrices)
  C <- matrix(0, 2, 2)
  C[lower.tri(C, diag=TRUE)] <- params[1:3]
...
# Grid search function for base BEKK parameters
grid_search_base_bekk <- function(returns, grid_points = 5) {
    ...
    for(a22 in a_values) {
      for(g11 in g_values) {
        for(g22 in g_values) {
          ...
          # Create initial parameter vector (without structural break parameters)
          init_params <- c(
            init_C[1,1], init_C[2,1], init_C[2,2],  # C matrix
...
format_matrix_with_se <- function(mat, se_mat, title) {
  cat("\n", title, ":\n")
  for(i in 1:nrow(mat)) {
    for(j in 1:ncol(mat)) {
...
X <- matrix(all_data$VIX_LAG)  # Only using lagged VIX as exogenous variable
colnames(X) <- "VIX_LAG"
...
grid_results <- grid_search_base_bekk(as.matrix(residuals), grid_points = 5)
...
# Fit base BEKK model with grid search optimized starting values
cat("\nFitting base BEKK model with QMLE estimation using optimized starting values...\n")
bekk_results <- base_bekk(as.matrix(residuals), all_data$VIX_LAG)

# Check convergence and print results
if(bekk_results$convergence == 0) {
  cat("\nBase BEKK Model converged successfully.\n")
  ...
  persistence <- calc_persistence(bekk_results$A, bekk_results$G)
  cat("\nPersistence Analysis:\n")
  cat(sprintf("Model persistence: %.3f %s\n", persistence,
              ifelse(persistence < 1, "(Stationary)", "(Non-stationary)")))
  
} else {
  cat("\nWarning: Base BEKK Model did not converge. Results may not be reliable.\n")
}

# Helper function to calculate persistence
calc_persistence <- function(A, G) {
...
n_params <- 11  # Total number of parameters in the base BEKK model
...
results <- data.frame(
  ...
  BTC_Returns = all_data$BTC[-1],
  ETH_Returns = all_data$ETH[-1],
  BTC_Vol = sqrt(h11),
  ETH_Vol = sqrt(h22),
...
write.csv(results, "base_model_results.csv", row.names = FALSE)
...
pdf("base_model_plots.pdf", width = 12, height = 10)
grid.arrange(p1, p2, p3, p4, ncol = 2, 
             top = textGrob("Cryptocurrency Market Dynamics",
                            gp = gpar(fontsize = 14, font = 2)))
dev.off()

# Also save as PNG for easier viewing
png("base_model_plots.png", width = 1200, height = 1000, res = 120)
grid.arrange(p1, p2, p3, p4, ncol = 2, 
             top = textGrob("Cryptocurrency Market Dynamics",
                            gp = gpar(fontsize = 14, font = 2)))
dev.off()
...
for(measure_name in names(measures)) {
  stats <- c(
    Mean = mean(measures[[measure_name]], na.rm = TRUE),
    SD = sd(measures[[measure_name]], na.rm = TRUE),
    Min = min(measures[[measure_name]], na.rm = TRUE),
    Max = max(measures[[measure_name]], na.rm = TRUE)
  )
...
write.csv(summary_stats, "base_model_summary_statistics.csv", row.names = FALSE)
